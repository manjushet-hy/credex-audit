...
export const runAlternativeToolAudit = (request: AuditRequest): AuditRecommendation[] => {
    const recommendations: AuditRecommendation[] = [];

    request.currentTools.forEach(tool => {
        const { vendor: currentVendor, plan: currentPlan } = getPlanDetails(tool.vendorId, tool.planId);
        if (!currentVendor || !currentPlan || currentPlan.pricePerUser === undefined) return;

        const currentMonthlyPerSeat = currentPlan.pricePerUser;
        ...
        ALL_VENDORS.forEach(altVendor => {
            ...
            // Find the cheapest plan in the alternative vendor that fits requirements
            ...
            if (viableAltPlans.length > 0) {
                ...
                if (monthlyDelta > (currentMonthlyPerSeat * tool.seats * 0.1)) {
                    recommendations.push({
                        id: generateUUID(),
                        createdAt: new Date().toISOString(),
                        recommendationCategory: 'cost',
                        ...
                        impactArea: 'cost',
                        riskLevel: 'medium',
                        triggeredBy: ['cheaper_competitor_found', 'use_case_overlap'],
                        blockedBy: [],
                        dependsOn: [],
                        targetToolInstanceId: tool.toolInstanceId,
                        targetVendorId: altVendor.vendorId,
                        targetPlanId: bestAltPlan.id,
                        monthlyDelta,
                        annualDelta: monthlyDelta * 12,
                        priorityScore: calculatePriorityScore('cost', monthlyDelta, 'medium'),
                        title: `Switch: ${currentVendor.vendorName} → ${altVendor.vendorName}`,
                        explanation: {
                            summary: `Replace ${currentVendor.vendorName} with ${altVendor.vendorName} to save $${monthlyDelta.toLocaleString()}/mo.`,
                            technicalReason: `The ${altVendor.vendorName} '${bestAltPlan.name}' plan provides equivalent ${currentVendor.category} capabilities at a lower price point.`,
                            businessReason: `Strategic vendor switch identified. You can maintain your ${currentVendor.supportedUseCases[0]} workflow while significantly reducing your SaaS burn.`
                        }
                    });
                }
            }
    ...
    // Sort by highest savings and return unique per target tool
    const uniqueSwitches = Array.from(new Map(recommendations.map(r => [r.targetToolInstanceId, r])).values());
...
export const generateAuditReport = (request: AuditRequest): AuditReport => {
    ...
    const alternativeRecs = runAlternativeToolAudit(request);

    // Dependency Graph Wiring
    securityRecs.forEach(sec => {
        // If a tool is being cancelled or switched, don't upgrade it
        const overlappingConsolidations = [
            ...consolidationRecs.filter(c => c.targetToolInstanceId === sec.targetToolInstanceId),
            ...alternativeRecs.filter(a => a.targetToolInstanceId === sec.targetToolInstanceId)
        ];
        sec.blockedBy = overlappingConsolidations.map(c => c.id);
    });

    rightSizingRecs.forEach(rs => {
        // If a tool is being cancelled or switched, don't right-size it
        const overlappingConsolidations = [
            ...consolidationRecs.filter(c => c.targetToolInstanceId === rs.targetToolInstanceId),
            ...alternativeRecs.filter(a => a.targetToolInstanceId === rs.targetToolInstanceId)
        ];
    ...
    let allRecommendations = [...consolidationRecs, ...securityRecs, ...rightSizingRecs, ...alternativeRecs];
